<?php 
 ...
 $prodcatsql1 = "SELECT emp_code FROM manage_emp WHERE emp_code = '$id'";
...
if(isset($_POST["submit"])){
	 ...
	 $leave_type = mysqli_real_escape_string($db,$_POST['leave_type']);
      ...
      $empsql = $db->query("SELECT * from emp_leaves WHERE emp_name LIKE '%$emp_name' OR  emp_name LIKE '$emp_name%' OR emp_name LIKE '%$emp_name%' AND leave_type= '$leave_type' AND status= '$status' AND lead_emp_code= '$id' AND leave_from BETWEEN '$from_date' AND '$from_date' And leave_to BETWEEN '$to_date' AND '$to_date' ORDER BY id Desc");
      $count = 0;
      if($empsql->num_rows > 0) {
		                                 ...
		                                 <td><?php echo ++$count;?></td>
											<td>
												<h2 class="table-avatar">
													<?php 
													$leavsql1 = "SELECT name,last_name,image FROM manage_emp WHERE emp_code = '".$emprow['emp_code']."'";
								$leav_results1 = mysqli_query($db, $leavsql1);
								$leavemp_data1 = $leav_results1->fetch_assoc();
													?>
													<a href="" class="avatar"><img src="profile-img/<?php echo $leavemp_data1['image']; ?>" alt=""></a>
													<a href=""><?php echo $leavemp_data1['name'];?> <?php echo $leavemp_data1['last_name'];?> (<?php echo $emprow['emp_code'];?>)</a>
												</h2>
											</td>
											<td><?php echo $emprow['leave_type'];?></td>
											<td><?php echo $emprow['leave_from'];?></td>
											<td><?php echo $emprow['leave_to'];?></td>
											<td><?php echo $emprow['days'];?> days</td>
											<td><?php echo $emprow['reason'];?></td>
		 ...
		 Echo "Not Found in Search";
										...
										<?php
									$sql = $db->query("select * from emp_leaves WHERE lead_emp_code= '$id' ORDER BY id DESC");
									$count = 0;
									if($sql->num_rows>0){
										...
										<?php
		                                 ...
		                                 <td><?php echo ++$count;?></td>
											<td>
												<h2 class="table-avatar">
													<?php 
													$leavsql2 = "SELECT name,last_name,image FROM manage_emp WHERE emp_code = '".$row['emp_code']."'";
								...
								$leavemp_data2 = $leav_results2->fetch_assoc();
													...
													<a href=""><?php echo $leavemp_data2['name'];?> <?php echo $leavemp_data2['last_name'];?> (<?php echo $row['emp_code'];?>)</a>
												</h2>
											</td>											
											...
											<td class="text-center">
												<div class="dropdown action-label">
													...
													<div class="dropdown-menu dropdown-menu-right">
														<a class="dropdown-item passingpenData" href="#" data-bs-toggle="modal" data-bs-target="#pending_leave" data-id="<?php echo $row['id'];?>" data-emp-id="<?php echo $row['emp_code'];?>" data-leave-type="<?php echo $row['leave_type'];?>" data-leave-from="<?php echo $row['leave_from'];?>" data-leave-to="<?php echo $row['leave_to'];?>" data-days="<?php echo $row['days'];?>" data-comment="<?php echo $row['pending_comment'];?>"><i class="fa-regular fa-circle-dot text-info"></i> Pending</a>
														...
														<a class="dropdown-item passingdecData" href="#"  data-bs-toggle="modal" data-bs-target="#decline_leave" data-id="<?php echo $row['id'];?>" data-emp-id="<?php echo $row['emp_code'];?>" data-leave-type="<?php echo $row['leave_type'];?>" data-leave-from="<?php echo $row['leave_from'];?>" data-leave-to="<?php echo $row['leave_to'];?>" data-days="<?php echo $row['days'];?>" data-comment="<?php echo $row['decline_comment'];?>"><i class="fa-regular fa-circle-dot text-danger"></i> Declined</a>
													</div>
													<?php } ?>
												</div>
											</td>
							...
							<div class="modal-body">
								...
								<div class="modal-btn delete-action">
											...
											<form id="emp_leave_approve" method="post">
									         ...
									         <input id="lead_emp_id" name="lead_emp_id" value="<?php echo $id; ?>" type="hidden">
									         <div class="row">
