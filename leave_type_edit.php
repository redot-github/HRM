 ...
 $file_no_of_days = mysqli_real_escape_string($db, $_POST["no_of_days"]);
  	...
  	$results = mysqli_query($db, $query);
            ...
            ('$file_emp_code','$file_name $file_last_name','$file_no_of_days', '$file_no_of_days')");
             ...
             $update = $db->query("UPDATE leave_types SET leave_type='$file_leave_type', based_on = '$file_based_on', no_of_days='$file_no_of_days', upload_on = NOW() WHERE id='$file_holi_id'");
             if($update){				 
                echo "<strong style='color:green;'>".ucwords($file_leave_type). " - ".ucwords($file_based_on). "</strong> has been updated successfully.";
            }else{
