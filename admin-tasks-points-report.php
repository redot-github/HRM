						...
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped custom-table leave-employee-table mb-0 datatable">
									...
									<tbody>
										<?php
									$sql = $db->query("select * from emp_tasks ORDER BY id DESC");
									$count = 0;
									if($sql->num_rows>0){
                                    while($row = $sql->fetch_assoc()){ ?>
										<tr>
											...
											<td><?php echo $row['lead_name'];?>(<?php echo $row['lead_emp_code'];?>)</td>
											<td><?php $a_emp_id = $row['to_emp'];
								                    ...
								                    <?php echo $pro_ass_data['name'];?> <?php echo $pro_ass_data['last_name'];?>(<?php echo $row['to_emp'];?>)
								            </td>
